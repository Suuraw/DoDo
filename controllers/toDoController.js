...
export const createToDo = async (req, res) => {
  try {
    ...
    const query = `
            INSERT INTO Todo (title, description, iscompleted, createdby,createdat)
            VALUES ($1, $2, $3, $4,$5) 
            RETURNING *;
        `;
    const values = [title, description, isCompleted, createdBy,createdAt];
    const result = await db.query(query, values);

    console.log(result.rows[0]);
    res
      .status(201)
      .send({ message: "Created New Task!", todo: result.rows[0] });
  } catch (err) {
    console.error(err);
...
export const updateToDo = async (req, res) => {
  try {
    ...
    const { title, description, isCompleted ,completedon} = req.body;
    ...
    if(completedon!==undefined)
    {
        const result = await db.query(
            `UPDATE Todo 
            SET iscompleted=$1,completedon=$2
            WHERE id=$3 
            RETURNING *`,
            [ isCompleted, completedon, id]
          );
          return res.send({ message: "ToDo list Updated!", todo: result.rows[0] })
    }
    const result = await db.query(
      `UPDATE Todo 
      ...
      WHERE id=$5 
      RETURNING *`,
      [title, description, isCompleted, completedOn, id]
    );
    // const query = `
    //     UPDATE Todo
    //     SET title = $1, description = $2, isCompleted = $3, completedOn = $4
